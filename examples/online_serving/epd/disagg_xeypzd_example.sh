...
chat_with_image() {
    python $CURRENT_DIR/chat_with_image.py \
        ...
        --prefill-addr-list $(for ((i=0; i<PREFILL_NUMBER; i++)); do echo -n "${PREFILL_ADDR_PREFIX}_$i "; done) \
        --decode-addr-list $(for ((i=0; i<DECODER_NUMBER; i++)); do echo -n "${DECODER_ADDR_PREFIX}_$i "; done) \
        --model-name $MODEL \
